...
for key in tqdm(dic_train):
    if dic_train[key]["pitch"] <= 84 and dic_train[key]["instrument_family_str"] == "bass":
        keys_train.append(key)
for key in tqdm(dic_valid):
    if dic_valid[key]["pitch"] <= 84 and dic_valid[key]["instrument_family_str"] == "bass":
        keys_valid.append(key)
for key in tqdm(dic_test):
    if dic_test[key]["pitch"] <= 84 and dic_test[key]["instrument_family_str"] == "bass":
...
with open("keys_bass_train.txt", "w+") as f:
    for key in keys_train:
        f.write(key + "\n")
with open("keys_bass_valid.txt", "w+") as f:
    for key in keys_valid:
        f.write(key + "\n")
with open("keys_bass_test.txt", "w+") as f:
